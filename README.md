...
등록한 HTML은 브라우저에서 바로 편집할 수 있고(좌측 코드 · 우측 실시간 미리보기),
저장하면 그 템플릿을 쓰는 모든 폼에 즉시 반영됩니다. 저장할 때마다 버전이 남아
언제든 되돌릴 수 있습니다. 관리 화면은 한국어 / English 를 지원합니다.
...
```bash
npm run shots       # 전 페이지 스크린샷 (라이트/다크)
...
| 템플릿 편집 | 제자리 저장 · 버전 기록 · 복원 · 필드 삭제 확인 · 운영 중 폼 즉시 반영 |
| 다국어 | 두 로케일의 키/플레이스홀더 일치, 전환이 실제로 UI를 바꾸는지 (E2E) |
